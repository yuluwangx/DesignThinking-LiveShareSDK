...
import { getPenScen, setPenScen } from './pen_token_scen'

/**
 * Live Share Host implementation used for local testing.
 */
export class TestLiveShareHostScen implements ILiveShareHost {
  ...
  public static create(
    ...
    return new TestLiveShareHostScen(
  ...
  public getFluidTenantInfo(): Promise<IFluidTenantInfo> {
    TestLiveShareHostScen.ensureWarned()
  ...
  public getFluidToken(containerId?: string): Promise<string> {
    return Promise.resolve(TestLiveShareHostScen.LOCAL_MODE_TEST_TOKEN)
  }
  ...
  private setLocalTestContainerId(containerId: string): void {
    if (this._setLocalTestContainerId) {
      console.log('penpenpen1')
      this._setLocalTestContainerId(containerId)
    } else {
      console.log('penpenpen2')
      setPenScen(containerId)
    }
  }
